...
public class UpgradeShop extends JFrame {
    ...
    static final int upgradeShopMenuWidth = ScreenSettings.PX_SCREEN_WIDTH * 2 / 10;
    //Height will be 30% since we already decided this previously
    static final int upgradeShopMenuHeight = ScreenSettings.PX_SCREEN_HEIGHT * 3 / 10;

    static final int upgradeShopMenuTopY = ScreenSettings.PX_SCREEN_HEIGHT * 3 /10;
    final static int upgradeShopMenuTopX = centerScreenX - (upgradeShopMenuWidth / 2);
    ...
    final static String upgradeShopString = "Upgrade type shit" ;
    static int textWidth = 0;

    public static void upgradeShopUI(Graphics2D g){

        UIConstants.setTransparent(g);
        //Rectangle
        g.setColor(UIConstants.boxColor);
        //x y width height
        g.fillRect(upgradeShopMenuTopX,
                upgradeShopMenuTopY,
                upgradeShopMenuWidth,
                upgradeShopMenuHeight);

        UIConstants.setOpaque(g);
        //Outline
        g.setColor(UIConstants.boxOutline);
        g.drawRect(upgradeShopMenuTopX,
                upgradeShopMenuTopY,
                upgradeShopMenuWidth,
                upgradeShopMenuHeight);
        ...
        g.drawString(upgradeShopString,
                (centerScreenX - textWidth),
                (upgradeShopMenuTopY + menuSpacing));
    }
    ...
    private static  void initTextWidth(Graphics2D g){
        ...
        System.out.println(m.stringWidth(upgradeShopString));
        textWidth = m.stringWidth(upgradeShopString) / 2;
    }
}